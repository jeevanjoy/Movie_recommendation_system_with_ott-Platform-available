...
function show_details(movie_details,arr,movie_title,my_api_key,movie_id){
  ...
  platforms=get_ott_details(movie_id);
  const uniqueProviderNames = [...new Set(platforms.providerNames)];
 ...
 // alert("hh"+platforms);
  details = {
    'title':movie_title,
      ...
      'provider_name':JSON.stringify(uniqueProviderNames),
  }
...
function get_ott_details(id) {
  ...
  $.ajax({
    ...
    url:'https://api.themoviedb.org/3/movie/'+id+'/watch/providers?api_key=',//your api
    async: false,
...
// Loop over regions
          for (const region in data.results) {
          i = i+1;
          const regionData = data.results[region];

          // Check if flatrate providers exist
  ...
  return {providerNames:providerNames};
}
